...
function handleActions(actions, initialState) {
  return handleReduxActions(
    reduce(actions, (reducer, handler, action) => {
...
function createReducers(stateContext, reducers, initialState) {
  return {
    [stateContext]: handleReduxActions(
      reduce(flatten(reducers), (reducer, action) => {
...
function createSelectorsA(context, keys) {
  ...
  return keys.map((key) => {
    return createSelector(stateSelector, state => isArray(key) ? state.getIn(key) : state.get(key));
  });
}
...
function createSelectors(context, keys) {
  ...
  return reduce(keys, (selectors, key) => {
    selectors[`${key}Selector`] = createSelector(stateSelector, state => state[key]);
    return selectors;
  }, {});
}
